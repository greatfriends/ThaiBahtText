...
namespace ThaiBahtText
{
  ...
  public static class ThaiBahtTextUtil
  {
    ...
    public static string ConvertToThaiBahtText(decimal? amount, ThaiBahtTextOptions options)
    {
      return amount.ThaiBahtText(options);
    }
    ...
    private static void SpeakTo(StringBuilder sb, string text, UsesEt mode)
    {
      ...
      for (int i = 0; i < length; i++)
      {
        ...
        else
        {
          ...
          {
            sb.Append(thaiNumbers[c]).Append(thaiPlaces[length - i]);
          }
        }
        lastc = c;
      }
